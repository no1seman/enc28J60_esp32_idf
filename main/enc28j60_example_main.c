...
// in: desired frequency in Mhz
// out: closest frequency to 25Mhz/N in Hz according to silicon errarta issue #1 (http://ww1.microchip.com/downloads/en/devicedoc/80349c.pdf)
int get_closest_frequency(int frequency)
{
    if (frequency < 1)
    {
        abort();
    ...
    {
        n++;
    } while (25 / n > frequency);

    return (abs(25.0 / n - frequency) > abs(25.0 / (n - 1) - frequency)) ? (int)(25.0 / (n - 1) * 1000 * 1000) : (int)(25.0 / n * 1000 * 1000);
}
